import matplotlib.pyplot as plt
import numpy as np
import matplotlib.mlab as mlab 
import pymysql as MySQLdb
from snownlp import SnowNLP
import time
import sys
TIMEBASE = 1452932340

db = MySQLdb.connect('localhost', 'root', '', 'consensus', charset = 'utf8')
cursor = db.cursor()
x_time = []
y_negativity = []

#sql = """select time, detail from weibo 
#        where identity='微博达人' or identity='官方认证' or identity='微博大V' 
#        order by time"""
#sql = """select time, detail from weibo order by time"""

sql = """select time, detail from weibo 
        where identity='官方认证'
        order by time"""
try:

    # 执行SQL语句
    cursor.execute(sql)
    # 获取所有记录列表
    results = cursor.fetchall()
    
except:
    print(sql) #print(sql)

time_dict={}
for row in results:
    #hour = int(((time.mktime(time.strptime(str(row[0]), "%Y-%m-%d %H:%M:%S")))-TIMEBASE)/3600)
    hour = int(((time.mktime(time.strptime(str(row[0]), "%Y-%m-%d %H:%M:%S")))-TIMEBASE)/86400)
    if hour in time_dict:
        time_dict[hour].append(1-(SnowNLP(row[1]).sentiments))
    else:
        time_dict[hour]=[]
        time_dict[hour].append(1-(SnowNLP(row[1]).sentiments))
'''
for key in time_dict:
    if key>30:
        break
    x_time.append(key)
    #情感走势
    #y_negativity.append(sum(time_dict[key])/len(time_dict[key]))
    #关注度走势
    y_negativity.append(len(time_dict[key]))
'''

for key in time_dict:
    x_time.append(key)
    #情感走势
    y_negativity.append(sum(time_dict[key])/len(time_dict[key]))
    #关注度走势
    #y_negativity.append(len(time_dict[key]))    



plt.figure(1)
plt.title("Offical-time" )
plt.xlabel("time(day)" )
plt.ylabel("offical" )
z1 = np.polyfit (x_time ,y_negativity , 3)
p1 = np.poly1d(z1)
yvals=p1(x_time) 
plt.plot(x_time, y_negativity ,"o",label='original values' )
plt.plot(x_time , yvals, 'r',label='polyfit values' )
plt.show()
##################
exit()
##############
y_num = []
x_phone=['苹果', '三星', '华为', '小米', 'OPPO', 'VIVO', '魅族', '其他']
sql = """select phone from weibo where phone like '%iPhone%'
        or phone like '%iPad%' """
cursor.execute(sql)
y_num[0] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%三星%'"""
cursor.execute(sql)
y_num[1] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%华为%' 
        or phone like '%荣耀%' """
cursor.execute(sql)
y_num[2] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%米%' """
cursor.execute(sql)
y_num[3] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%OPPO%'
        or phone like '%oppo%' """
cursor.execute(sql)
y_num[4] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%vivo%'
        or phone like '%VIVO%' """
cursor.execute(sql)
y_num[5] = len(cursor.fetchall())

sql = """select phone from weibo where phone like '%魅%' """
cursor.execute(sql)
y_num[6] = len(cursor.fetchall())

sql = """select phone from weibo """
cursor.execute(sql)
y_num[7] = len(cursor.fetchall()) - sum([y_num[i] for i in y_num[:6]])

fig = plt.figure()  
plt.bar(x_phone,y_num,0.4,color="green")  
plt.xlabel("pyhone")  
plt.ylabel("number")  
plt.title("bar chart")  
    
  
plt.show()    